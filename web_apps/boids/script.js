...
class BoidsDemo extends Demo {
  constructor() {
    ...
    this.spatialGrid.cells = new Array(
      this.spatialGrid.width * this.spatialGrid.height
    );
  ...
  updateBoidGrid(_boid) {
    ...
    if (_boid == this.debugBoid) {
      ...
      ctx.beginPath();
      ctx.rect(
        gridX * this.spatialGrid.cellSize,
        gridY * this.spatialGrid.cellSize,
    ...
    if (_boid.gridId != gridId) {
      ...
      if (_boid.gridId != null)
        this.spatialGrid.cells[_boid.gridId].splice(
          this.spatialGrid.cells[_boid.gridId].indexOf(_boid),
          1
        );
      //change id
      _boid.gridId = gridId;
  ...
  mouseDown() {
    if (this.debugBoid == null) this.debugBoid = this.boids[0];
    else this.debugBoid = null;
  }
  update(_delta) {
    super.update();
    for (let i = 0; i < this.boids.length; i++) {
      ...
      boid.angle +=
        angleDiff > 0 ? this.boidAngularSpeed : -this.boidAngularSpeed;
    ...
    for (let i = 0; i < this.boids.length; i++) {
      ...
      if (nearBoids.length > 0) {
        ...
        calculatedVector = Vector2.add(
          calculatedVector,
          this.separationVector(boid, nearBoids)
        );
        calculatedVector = Vector2.add(
          calculatedVector,
          this.alignmentVector(boid, nearBoids)
        );
        calculatedVector = Vector2.add(
          calculatedVector,
          this.cohesionVector(boid, nearBoids)
        );
  ...
  separationVector(_boid, _nearBoids) {
    ...
    for (let i = 0; i < _nearBoids.length; i++) {
      ...
      nearBoidsVector = Vector2.add(
        nearBoidsVector,
        Vector2.mult(vu, 1 / Vector2.magSqr(v))
      );
      /*if(_boid == this.debugBoid){
				ctx.beginPath();
    ...
    if (_boid == this.debugBoid) {
      ...
      ctx.moveTo(_boid.pos.x, _boid.pos.y);
      ctx.lineTo(
        _boid.pos.x + nearBoidsVector.x * this.boidSize * 1.5,
        _boid.pos.y + nearBoidsVector.y * this.boidSize * 1.5
      );
      ctx.stroke();
    }
  ...
  stayOnScreenVector(_boid) {
    const toCenter = Vector2.create(
      canvas.width / 2 - _boid.pos.x,
      canvas.height / 2 - _boid.pos.y
    );
  ...
  alignmentVector(_boid, _nearBoids) {
    ...
    if (_boid == this.debugBoid) {
      ...
      ctx.moveTo(_boid.pos.x, _boid.pos.y);
      ctx.lineTo(
        _boid.pos.x + steerVector.x * this.boidSize * 1.5,
        _boid.pos.y + steerVector.y * this.boidSize * 1.5
      );
      ctx.stroke();
    }
  ...
  getSurroundingCells(_boid) {
    ...
    if (bV.x == 1 && bV.y == -1) {
      ...
      if (canRight) {
        ...
        if (canUp)
          r.push(
            this.spatialGrid.cells[_boid.gridId + 1 - this.spatialGrid.width]
          );
      }
    }
    //left up
  ...
  doSegmentsIntersect(A, B) {
    ...
    if (
      intersectionPoint.x <= maximumX &&
      intersectionPoint.x >= minimumX &&
      intersectionPoint.y <= maximumY &&
      intersectionPoint.y >= minimumY
    )
      return true;
    return false;
  }
