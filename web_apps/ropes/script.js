...
class FabrikDemo extends Demo{
	...
	update(_delta){
		...
		if(MOUSE.down){
			if(MOUSE.button == 0)
				this.target = Vector2.create(MOUSE.x, MOUSE.y);
			else
				this.origin = Vector2.create(MOUSE.x, MOUSE.y);
			this.fabrik();
		}
	}
	...
	fabrik(){
		...
		for(let i = this.pointCount-1; i > 0; i--)
			this.points[i-1] = Vector2.add(this.points[i], Vector2.mult(Vector2.unit(Vector2.sub(this.points[i-1], this.points[i])), this.length));
...
class SnakeDemo extends Demo{
	constructor(){
		...
		this.points.push(Vector2.create(canvas.width/2, canvas.height/2));
		for(let i = 1; i < this.pointCount; i++){
			this.points.push(Vector2.add(this.points[this.points.length-1], Vector2.mult(Vector2.fromAngle(a), this.length)));
			a += ad;
			ad *= 0.975;
		}
	}
	...
	update(_delta){
		...
		for(let i = 0; i < this.pointCount;i++){
			ctx.beginPath();
			ctx.arc(this.points[i].x, this.points[i].y, this.circleRadius, 0, 2 * Math.PI);
			ctx.stroke();
		}
		if(MOUSE.down){
			this.fabrik();
	...
	fabrik(){
		...
		for(let i = this.pointCount-1; i > 0; i--){
			...
			if(i < this.pointCount-1){
				...
				if(aBetween < 0){
					if(aBetween > -this.limitAngle)
						aNext = aPrev - this.limitAngle;
				}
				else{
					if(aBetween < this.limitAngle)
						aNext = aPrev + this.limitAngle;
			...
			this.points[i-1] = Vector2.add(this.points[i], Vector2.mult(Vector2.fromAngle(aNext), this.length))
		}
...
class RopeLikeDemo extends Demo{
	...
	createStick(_a, _b){
		const s = {a: _a, b: _b, length: Vector2.dist(_a.pos, _b.pos), destroyed: false};
		_a.sticks.push(s);
		_b.sticks.push(s);
		this.sticks.push(s);
	}
	...
	updatePoints(_delta){
		for(let i = 0; i < this.points.length; i++){
			...
			if(!point.locked){
				...
				point.pos = Vector2.add(point.pos, Vector2.sub(point.pos, point.prevPos));
				point.pos = Vector2.add(point.pos, Vector2.create(0, 1000 * _delta * _delta));
...
class RopeDemo extends RopeLikeDemo{
	...
	buildSample(){
		...
		this.createStick(this.points[this.points.length-1], this.points[this.points.length-2]);
		this.createStick(this.points[this.points.length-1], this.points[this.points.length-3]);
		this.createStick(this.points[this.points.length-2], this.points[this.points.length-3]);
	}

	setInfo(){
		infoDiv.innerHTML = "normal mode:<br> LMB on point to move it<br> RMB on point to lock it<br>press spacebar to toggle edit mode <br> edit mode:<br> LMB on point to create stick <br> LMB off points to create a point <br> RMB on point to remove it";
	}
...
class ClothDemo extends RopeLikeDemo{
	...
	buildSample(){
		const width = 16;
		const height = 10;
		const length = Math.round(Math.min(0.9*canvas.width/width, 0.9*canvas.height/(height+1)));

		for(let y = 0; y < height; y++)
		for(let x = 0; x < width; x++){
			this.createPoint(canvas.width/2 - length*width/2 + x*length, length + y*length);
		}

		for(let x = 0; x < width; x+=3)
			this.points[x].locked = true;

		for(let y = 0; y < height; y++)
		for(let x = 0; x < width; x++){
			if(x != width-1)
				this.createStick(this.points[x+y*width], this.points[x+y*width+1]);
			if(y != height-1)
				this.createStick(this.points[x+y*width], this.points[x+y*width+width]);
		}
	}
...
class StringDemo extends RopeLikeDemo{
	constructor(){
		...
		for(let i = 0; i < this.points.length; i++){
			...
			Object.assign(p, {ball: Ballz.createBall(p.pos.x, p.pos.y, 10, {acc: Vector2.create(0, 1000) })});
			// console.log(p);
		}
		// console.log(this.points[0]);
		this.collider = Ballz.createBall(canvas.width/2, canvas.height/2, 120, {static: true});
	}
	...
	buildSample(){
		...
		for(let x = 0; x < width; x++){
			this.createPoint(canvas.width/2 - length*width/2 + x*length, length + length);
		}

		for(let x = 0; x < width; x++){
			if(x != width-1)
				this.createStick(this.points[x], this.points[x+1]);
		}
	}
	...
	render(){
		...
		ctx.beginPath();
		ctx.arc(this.collider.pos.x, this.collider.pos.y, this.collider.radius, 0, 2 * Math.PI);
		ctx.stroke();
...
function setup(){
	...
	canvas.addEventListener('mousedown', (e) => {MOUSE.down = true; MOUSE.x = e.offsetX; MOUSE.y = e.offsetY; MOUSE.button = e.button; demo.mouseDown();} );
	canvas.addEventListener('mousemove', (e) => {MOUSE.x = e.offsetX; MOUSE.y = e.offsetY;} );
	canvas.addEventListener('mouseup', (e) => {MOUSE.down = false; demo.mouseUp();} );
	canvas.addEventListener('mouseleave', (e) => {MOUSE.down = false;} );
	document.addEventListener('keydown', (e) => {
		switch(e.key){
			case '1': demo = new FabrikDemo(); break;
			case '2': demo = new SnakeDemo(); break;
			case '3': demo = new RopeDemo(); break;
			case '4': demo = new ClothDemo(); break;
			// case '5': demo = new StringDemo(); break;
			default: demo.keyDown(e.key); break;
		}
	});
	document.addEventListener('keyup', (e) => {demo.keyUp(e.key);});
